...
export default class Crawler {
  ...
  async scraping(page: puppeteer.Page, keyword: string) {
    await page.goto(`https://www.medium.com/tag/${keyword}/recommended`, {
      waitUntil: "domcontentloaded",
    });

    return await this.getArticles(page);
  }
  ...
  start = async () => {
    ...
    const keywordResults = await database.getAllData("keywords");

    const articles: Article[] = [];

    for (const result of keywordResults) {
      const articleResults = await this.scraping(page, result.keyword);
      const mergeKeyword = articleResults.map((articleResult) => ({
        ...articleResult,
